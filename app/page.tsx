...
export default function Home() {
  ...
  const add = async (list: FileList | File[]) => {
    ...
    const accepted = files.filter((file) => (EXTENSIONS.test(file.name) || file.type.startsWith("image/") || file.type.startsWith("video/")) && file.size <= (isVideo(file) ? 500 : 75) * 1024 * 1024).slice(0, Math.max(0, capacity));
    if (accepted.length !== files.length) setNotice("Solo se admiten HEIC, JPEG, PNG, WebP, MOV y MP4; máximo 75 MB por foto y 500 MB por vídeo.");
  ...
  const generate = async (requested?: string[]) => {
    ...
    try {
      ...
      let pendingIds = ids;
      while (pendingIds.length && runId.current === generation) {
        ...
        console.info(`[media:${traceId}] ${item.filename} · análisis solicitado (intento ${attempt})`);
        try {
          ...
          if (cancelled) console.info(`[media:${traceId}] ${item.filename} · análisis cancelado`); else console.error(`[media:${traceId}] ${item.filename} · análisis fallido`, cause);
          if (runId.current === generation) {
            const message = cause instanceof Error ? cause.message : "Error inesperado.";
            if (!cancelled && attempt < MAX_ANALYSIS_ATTEMPTS) {
              retryIds.push(item.id);
              update(item.id, { status: "queued", error: `Reintentando en 1 segundo… (${attempt}/${MAX_ANALYSIS_ATTEMPTS})` });
            } else update(item.id, { status: cancelled ? "queued" : "error", error: message });
          }
        }
        finally { if (aborters.current.get(item.id) === controller) aborters.current.delete(item.id); }
      }
        if (!retryIds.length || runId.current !== generation) break;
        setNotice(`Reintentando ${retryIds.length} archivo${retryIds.length === 1 ? "" : "s"}…`);
  ...
  const downloadJson = () => {
    ...
    link.href = url;
    link.download = `media-tag-optimizer-${exportedAt.toISOString().replace(/[:.]/g, "-")}.json`;
    link.click();
  ...
  const download = async () => {
    ...
    try {
      ...
      const link = document.createElement("a"); link.href = `/api/media/export/${exportId}/download`; link.download = `media-tag-optimizer-${new Date().toISOString().slice(0, 10)}.zip`; link.click(); setNotice("ZIP listo; descarga iniciada.");
    } catch (cause) {
  ...
  return <main className="min-h-screen bg-[#f5f0e8] text-[#24231e]"><div className="mx-auto max-w-7xl px-5 py-6 sm:px-8 lg:px-12"><header className="flex items-center justify-between border-b border-[#24231e]/15 pb-5"><div className="flex items-center gap-3"><img src="/media-tag-optimizer.png" alt="MediaTagOptimizer" className="size-10 rounded-xl" /><div><p className="font-serif text-2xl leading-none">MediaTagOptimizer</p><p className="mt-1 text-[10px] font-bold uppercase tracking-[.18em] text-[#6f7551]">Optimización de metadatos</p></div></div>{items.length > 0 && <span className="rounded-full bg-[#edf0e3] px-3 py-1 text-xs font-semibold">{items.length} archivos</span>}</header>
    ...
    <section className="grid gap-4 lg:grid-cols-[1.45fr_.55fr]"><div onDragOver={(event) => event.preventDefault()} onDrop={onDrop} className="relative min-h-64 rounded-[2rem] border border-dashed border-[#b85c3e]/50 bg-[#fffaf1] p-8"><ImagePlus size={24} /><h2 className="mt-8 font-serif text-3xl">Suelta tus fotos y vídeos</h2><p className="mt-2 text-sm leading-6 text-[#625e55]">HEIC, JPEG, PNG, WebP, MOV o MP4 · hasta 75 MB por foto y 500 MB por vídeo</p><button disabled={exporting} onClick={() => input.current?.click()} className="mt-6 inline-flex items-center gap-2 rounded-full bg-[#b85c3e] px-5 py-3 text-sm font-bold text-white disabled:opacity-45"><Upload size={16} /> Elegir archivos</button><input ref={input} onChange={onChange} className="hidden" type="file" accept={ACCEPT} multiple /></div>
