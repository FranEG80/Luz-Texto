...
import { AlertCircle, Check, Clipboard, Download, FileVideo, ImagePlus, LoaderCircle, RefreshCcw, Sparkles, Trash2, Upload, X } from "lucide-react";
...
export default function Home() {
  ...
  const download = async () => {
    ...
    try {
      const form = new FormData(); selectedItems.forEach((item) => form.append("media", item.file)); form.append("manifest", JSON.stringify(selectedItems.map(({ id, file, filename, caption, title, keywords }) => ({ id, filename, caption, title, keywords, fallbackDateTime: fileDateTime(file) })))); form.append("convertToWebp", String(convertToWebp)); form.append("renameByDate", String(renameByDate));
      const response = await fetch("/api/media/export", { method: "POST", body: form }); if (!response.ok) throw new Error((await response.json() as { error?: { message?: string } }).error?.message ?? "No se ha podido crear el ZIP.");
      const url = URL.createObjectURL(await response.blob()); const link = document.createElement("a"); link.href = url; link.download = `luz-y-texto-${new Date().toISOString().slice(0, 10)}.zip`; link.click(); URL.revokeObjectURL(url); setNotice("ZIP listo.");
    } catch (cause) { setNotice(cause instanceof Error ? cause.message : "No se ha podido crear el ZIP."); }
